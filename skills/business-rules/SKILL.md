...
- **Patients list `lastVisit`:** Latest `scheduled_at` among appointments that are **`completed`**, **`is_active`**, and **`scheduled_at < now()`** (DB); `assignedDoctor` matches that row. Full rule: `docs/08-Business-Rules.md`.
