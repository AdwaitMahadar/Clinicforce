...
- **Appointment category:** `general`, `orthopedic`, `physiotherapy`. **Visit type:** `general`, `first-visit`, `follow-up-visit`. **`title`** optional in DB; only admin/doctor see the form field — staff create stores null, staff update cannot mutate `title`, staff reads get `title: null` from detail/list/patient nested/search/home actions (DB unchanged). **`fee`** optional `numeric(10,2)`; all roles; UI shows ₹ + fixed decimals (`formatAppointmentFeeInr`). Heading format: `Category - Visit Type` or `Category - Visit Type (Title)` via `formatAppointmentHeading`.
