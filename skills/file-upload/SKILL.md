...
- **Code:** Shared client in `lib/storage/s3-client.ts`; object keys via `lib/storage/document-object-key.ts` (`{clinicSubdomain}/{assignedToType}/{assignedToId}/…`). `clinicSubdomain` is on `session.user` — tenant pipeline in `docs/05-Authentication.md` §4. Zod in `lib/validators/document.ts` (`assignedToId` is non-empty string, not `.uuid()`, when the assignee can be a user with a non-UUID Better-Auth id).
