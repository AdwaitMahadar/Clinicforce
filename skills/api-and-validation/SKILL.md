...
- **`getPatients` list rows:** Each row has `status: "active" | "inactive"` only (mapped from DB in the query). There is no `isActive` on that payload — UI mappers must pass through `row.status`, not `row.isActive`.
