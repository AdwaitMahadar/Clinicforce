...
- **Detail tab slice actions** (detail streaming): `getPatientDetail{Documents,Appointments,Prescriptions}Tab` and `getAppointmentDetail{Documents,Appointments,Prescriptions}Tab` in `lib/actions/patients.ts` / `appointments.ts` mirror monolithic tab RBAC; **`lib/detail-tab-fetch-cache.ts`** holds the only **`React.cache`** wrappers per slice (prefetch + tab loaders import those exports only). **`loadAppointmentDetailViewData`** / **`loadPatientDetailViewData`** (`*_lib/*-detail-view-data.ts`) bundle blocking shell data for **`view/[id]`** pages + intercept modals — not new actions, just **`get*DetailCore`** (+ **`getActiveDoctors`** in parallel for appointments) + mappers.
...
- **Do not** add **`React.cache`** wrappers for **`get*Detail*Tab`** calls outside **`lib/detail-tab-fetch-cache.ts`** — prefetch + loaders must share one memo table per slice.
