...
*   `<DocumentCard />` / `<UploadDocumentDialog />` - Patient/appointment document list + presigned upload (see `skills/file-upload/SKILL.md`). **`DocumentCard`** — inline delete when **`uploadDocument`** (red trash→**Delete?** pill, **`deleteDocument`**). **`DocumentsTab`** — sidebar grid + Upload + dialog; **`patientId`** + optional **`appointmentId`**. **`AppointmentListTab`** — patient visit cards → **`/appointments/view/[id]`**; optional **`currentAppointmentId`** for **Current** on appointment detail.
