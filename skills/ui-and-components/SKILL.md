...
*   **Medicines**: 
    ...
    *   Medicine **detail** (`MedicineDetailPanel`): saving while **`isActive === false`** opens an **`AlertDialog`** (`radix-ui`); confirm → **`updateMedicine`** with **`isActive: true`** + form fields, then **`useDetailExit`** (same as other saves).
