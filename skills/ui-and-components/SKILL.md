...
*   **`PrescriptionsTab`** (`components/common/PrescriptionsTab.tsx`) + **`components/common/prescriptions/`** — appointment-panel Rx tab (admin/doctor, `viewPrescriptions`). Tab file owns state, actions, clear dialog; **`DraftPrescriptionShell`** (`collapsible` + chevron toggles body + notes; Clear/Publish outside toggle; same accordion height animation as published), **`DraftMedicineCard`** / **`DraftSortableCard`**, **`DoseSlotRow`**, **`PublishedPrescriptionAccordion`**, **`RxClinicalDocument`**, **`prescription-slots`**, **`prescription-tab-helpers`**. Draft shell border: `var(--color-border)`. Same optimistic rules (`DoseSlotRow` debounce guards, BF/AF `motion.span`, focused duration/remarks refs, optimistic reorder + `flushDebounced` `inFlight` set).
*   **`PatientPrescriptionsTab`** (`components/common/PatientPrescriptionsTab.tsx`) — patient-panel Rx tab (published only). Accordion rows; expanded body → **`RxClinicalDocument`** (`layout="plain"`). **No** navigation to appointments. `PatientPrescriptionSummary.items` from `getPrescriptionsByPatient`; drafts never listed.
