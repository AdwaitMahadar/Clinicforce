...
*   **`PrescriptionsTab`** (`components/common/PrescriptionsTab.tsx`) + **`components/common/prescriptions/`** — appointment-panel Rx tab (admin/doctor, `viewPrescriptions`). Tab file owns state, actions, clear dialog; **`DraftPrescriptionShell`**, **`DraftMedicineCard`** / **`DraftSortableCard`**, **`DoseSlotRow`**, **`PublishedPrescriptionAccordion`**, **`RxClinicalDocument`**, **`prescription-slots`**, **`prescription-tab-helpers`** live under `prescriptions/`. Draft shell border: `var(--color-border)` (matches nested line cards). Same optimistic rules as before (`DoseSlotRow` debounce guards, BF/AF `motion.span`, focused duration/remarks refs, optimistic reorder + `flushDebounced` `inFlight` set).
