...
*   `<DocumentCard />` / `<UploadDocumentDialog />` - Patient/appointment document list + presigned upload (see `skills/file-upload/SKILL.md`). **`DocumentCard`** — inline delete when **`uploadDocument`** (red trash→**Delete?** pill, **`deleteDocument`**). **`DocumentsTab`** — detail-panel tab grid + Upload + dialog; **`patientId`** + optional **`appointmentId`** (upload link only; list is all patient docs). **`AppointmentListTab`** — patient visit cards → **`/appointments/view/[id]`**; optional **`currentAppointmentId`** for **Current** on appointment detail.
...
*   `<AsyncSearchCombobox />` - Popover + cmdk; **`fetchItems(query)`** async (debounced internally); **`shouldFilter={false}`**; **`modal={false}`** for dialogs; optional **`renderOption`**. Appointment patient: **`AppointmentPatientCombobox`** + **`searchPatientsForPicker`** (limit 8, same search columns as `getPatients`). Prescription medicine line items: **`searchMedicinesForPicker`** (limit 8; name/brand/category; optional **`excludeIds`** for already-added medicines).
*   **`PrescriptionsTab`** (`components/common/PrescriptionsTab.tsx`) — appointment-panel Rx tab (admin/doctor, `viewPrescriptions`). **Draft state:** medicine cards with `@dnd-kit` drag-to-reorder; each card has a section header bar, a medicine `AsyncSearchCombobox` + Days input row, a `grid-cols-3` dose-slot grid (Morning/Afternoon/Night), and an italic remarks textarea footer. Per-change server actions + 350ms debounced qty patches — errors via Sonner. **Published state:** collapsible accordion → read-only ℞ clinical document. **Optimistic UI rules:** (1) Each `DoseSlotRow` holds `draftEnabled`/`draftQty`/`draftMeal` local state, each guarded by a `pending*Ref`. The shared `useEffect([enabled, quantity, meal, itemUpdatedAt])` clears a draft only when the server-confirmed prop has caught up to the last-queued ref value — mid-flight responses from earlier clicks cannot stomp newer optimistic values. (2) The BF/AF pill is a single always-mounted `<motion.span>` animating `x` between `"0%"` (Before food) and `"100%"` (After food) — never conditionally mounted, so Framer Motion never queues stale animation history on rapid clicks. (3) Days / Remarks fields are protected by `durationFocusedRef` / `remarksFocusedRef` — the `useEffect` reset is skipped while the field is focused, preventing a save from another field from wiping in-progress typing. (4) `onDragEnd` applies `setRx(newOrder)` optimistically before the `await`; rolls back to `prevItems` on failure. (5) `flushDebounced` uses an `inFlight: Set<string>` ref to skip execution if a call for the same `itemId` is already running, preventing concurrent server calls from racing each other. Publish action promotes draft → published; clear action removes all items.
*   **`PatientPrescriptionsTab`** (`components/common/PatientPrescriptionsTab.tsx`) — patient-panel Rx tab (published prescriptions only, `getPrescriptionsByPatient`). Each prescription renders as a **collapsible accordion card** — collapsed header shows Rx# (small muted mono, top line), doctor name (large bold, second line), visit date and medicine count badge on the right. Expanded body shows the same inline ℞ clinical document as the appointment tab — **no navigation to `/appointments/view/[id]`**; all data is shown in-place. The `PatientPrescriptionSummary` type includes an **`items`** array (`PrescriptionItemForAppointmentTab[]`) fetched by `getPrescriptionsByPatient` alongside the summary so no extra round-trip is needed. Draft prescriptions never appear.
*   `<DetailPanel />` / `<DetailPanelTabs />` / `<DetailSidebar />` / **`AppointmentPatientSummaryCard`** (`appointments/_components/`) — Detail shell: header; **tabbed main column** (Details + optional Documents/Appointments/**Prescriptions**; Framer sliding underline; optional **`detailsTabIcon`**; tab bar hidden when only Details); optional **right sidebar** (`min(26.25rem,36vw)` max **390px** — space taken from `flex-1` main column, **not** modal **`modal-shell-sizes`** width); **`sidebarTop`** → **`DetailSidebar`** **`topSlot`** (appointment patient card) + activity log; footer (Save / Cancel / optional delete). With **`topSlot`**, sidebar uses **40/60** vertical split; slot scrolls on overflow. Right column hidden when `isCreate=true` OR when the user lacks `viewDetailSidebar` (staff) — `DetailPanel` reads this via `usePermission` (staff: no Documents/Appointments tabs). **Prescriptions** tab: optional **`prescriptionsTab`** + **`viewPrescriptions`** (admin/doctor) — **`PrescriptionsTab`** (appointment) or **`PatientPrescriptionsTab`** (patient; published rows only). View modals use `size="lg"` for staff vs `"xl"` for admin/doctor (`modal-shell-sizes.ts`). **Phase 6+:** `DetailPanel` forwards `events` / `hasMoreEvents` / `entityType` / `entityId` / optional `sidebarTop` to `DetailSidebar`; `DetailSidebar` keeps the same infinite-scroll activity pattern as before. Entity panels pass `documentsTab` / `appointmentsTab` / `prescriptionsTab` when applicable.
