...
*   **`PrescriptionsTab`** / **`PatientPrescriptionsTab`** — structured Rx on appointment (draft → publish, **`@dnd-kit`** reorder, per-change server actions / debounced qty — errors via Sonner); patient tab = published history only (`docs/06-UI-Design-System.md` shared tab bodies).
