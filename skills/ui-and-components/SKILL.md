...
*   **`PrescriptionsTab`** (`components/common/PrescriptionsTab.tsx`) — appointment-panel Rx tab (admin/doctor, `viewPrescriptions`). **Draft state:** medicine cards with `@dnd-kit` drag-to-reorder; each card has a section header bar (index badge + "MEDICINE" label left, hover-reveal grip/delete right), a medicine `AsyncSearchCombobox` + Days input row, a `grid-cols-3` dose-slot grid (Morning/Afternoon/Night — checkbox + centered icon+label + qty stepper on line 1; full-width animated BF/AF Framer Motion `layoutId` slider on line 2; neutral `surface-alt` background, color on border/label/slider only), and an italic remarks textarea footer. Per-change server actions + debounced qty patches — errors via Sonner. **Published state:** collapsible accordion with header (Rx# reference small top, doctor name bold below, visit date + medicine count right); expanded body shows a read-only ℞ clinical document (two-column: name + remarks left, dose pills + "for N days" badge right). Publish action promotes draft → published; clear action removes all items.
