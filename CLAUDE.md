...
components/
  ...
  common/                   ← Domain components (see below)
    prescriptions/          ← Rx tab modules: draft shell, draft line cards, dose rows, published accordion, `prescription-published-history-list`, `RxClinicalDocument`, slot config + helpers (imported by `PrescriptionsTab.tsx` + `PatientPrescriptionsTab.tsx`)
...
- `PrescriptionsTab.tsx` — Appointment detail only (`viewPrescriptions`): three states (no Rx / draft / published); published top block uses **`PublishedPrescriptionAccordion`** with **`highlightCurrent`** (blue border + inline Shadcn **`Badge`** **Current** — same tokens as **`AppointmentListTab`**); **Other prescriptions** (eyebrow + list; dedupes open visit) = **`PrescriptionPublishedHistoryList`** (card border `var(--color-border)`); **`router.refresh()`** after publish; composes `common/prescriptions/` (`DraftPrescriptionShell` **`collapsible`**, **`DraftMedicineCard`**: medicine combobox (header-tinted trigger) + Days + remarks row + `DoseSlotRow`, helpers); draft shell `var(--color-border)`; `searchMedicinesForPicker`, slot auto-save, `@dnd-kit` reorder
- `PatientPrescriptionsTab.tsx` — Patient detail only (`viewPrescriptions`): read-only **published** rows via shared **`PrescriptionPublishedHistoryList`** (same data as **`getPatientDetail.prescriptions`** / patient **`PatientPrescriptionsTabLoader`** — drafts excluded); **`RxClinicalDocument`** — **no** row navigation to appointments
- `AppointmentListTab.tsx` — Detail panel appointments list: `PatientAppointment` cards (glass + shadow); non-current = accordion (full-width toggle, chevron flush right on date row); accordion **status** + expanded **category / visit / fee** are read-only **`span`**s sharing the same **`px-2.5`** shell as **Open** (**`PatientOpenPill`**); **`currentAppointmentId`** = flat card (no **Open** in body); **Current** = inline Shadcn **`Badge`** + blue tokens (same pattern as **`PublishedPrescriptionAccordion`**)
...
- `skeletons/` — Route `loading.tsx` building blocks (`PageHeaderSkeleton`, `TableDashboardSkeleton`, …, `DetailPageSkeleton`); **`DetailPanelTabSkeleton`** for **`DetailPanel`** inner tab **`Suspense`** fallbacks; **`ModalDetailPanelBodySkeleton`** for intercept modal inner **`Suspense`** fallbacks (tab-strip + form + right column widths mirror **`DetailPanel`** when **`viewDetailSidebar`**); **`ModalDetailSkeleton`** for rare full-modal + backdrop loading (`size` / `variant`)
...
- **`AppointmentDetailPanel` (edit):** **Documents** / **Appointments** / **Prescriptions** tab bodies are async RSC loaders (`appointments/_components/detail-tabs/*`) fed by tab slice actions + **`lib/detail-tab-fetch-cache.ts`** (same data as monolithic **`getAppointmentDetail`**). **Sidebar** uses **`getAppointmentDetailCore`** + **`buildAppointmentDetailCore`** (`AppointmentPatientSummaryCard` + activity).
- **`PatientDetailPanel` (view):** Tab order Details → Documents → Appointments → **Prescriptions** (admin/doctor — `viewPrescriptions`). Tab bodies use **`patients/_components/detail-tabs/*`** loaders + the same cache module; shell uses **`getPatientDetailCore`** / **`buildPatientDetailCore`**. **Published only** on Rx (`getPrescriptionsByPatient`); no navigation to `/appointments/view/[id]`.
