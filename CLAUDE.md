...
- `PrescriptionsTab.tsx` — Appointment detail only (`viewPrescriptions`): orchestrates draft/published Rx; composes `common/prescriptions/` (`DraftPrescriptionShell`, `DraftMedicineCard` / `DraftSortableCard`, `DoseSlotRow`, `PublishedPrescriptionAccordion`, helpers); draft shell uses `var(--color-border)` outline like line cards; `searchMedicinesForPicker`, slot auto-save, `@dnd-kit` reorder
