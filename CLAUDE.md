...
components/
  ...
  common/                   ← Domain components (see below)
    prescriptions/          ← Rx tab modules: draft shell, draft line cards, dose rows, published accordion, `RxClinicalDocument`, slot config + helpers (imported by `PrescriptionsTab.tsx` + `PatientPrescriptionsTab.tsx`)
...
- `PrescriptionsTab.tsx` — Appointment detail only (`viewPrescriptions`): orchestrates draft/published Rx; composes `common/prescriptions/` (`DraftPrescriptionShell` with **`collapsible`** draft accordion, `DraftMedicineCard` / `DraftSortableCard`, `DoseSlotRow`, `PublishedPrescriptionAccordion`, helpers); draft shell uses `var(--color-border)` outline like line cards; `searchMedicinesForPicker`, slot auto-save, `@dnd-kit` reorder
- `PatientPrescriptionsTab.tsx` — Patient detail only (`viewPrescriptions`): read-only **published** prescription accordions (`getPatientDetail.prescriptions` / `getPrescriptionsByPatient` — drafts excluded); expanded body uses shared `RxClinicalDocument` — **no** row navigation to appointments
