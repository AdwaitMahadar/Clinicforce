...
The top nav also exposes **global search** (`components/common/UniversalSearch.tsx`): a `Dialog` + cmdk `Command` palette (⌘/Ctrl+K) that calls `searchGlobal` and navigates to entities or opens documents via presigned URL when permitted. **Medicines** and **Documents** result groups are shown only when the user has **`viewMedicines`** and **`viewDocuments`** respectively (staff see neither group).
...
Popover (`modal={false}` for use inside intercepting `Dialog`s) + shadcn **`Command`** / cmdk with **`shouldFilter={false}`**. Callers pass a debounced async **`fetchItems(query)`**; results render as **`CommandItem`**s with a scroll-capped **`CommandList`** (default ~8 rows via `max-h` + `overflow-y-auto`). Generic **`getOptionValue` / `getOptionLabel`** (and optional **`renderOption`**) keep it reusable for other entities. Optional **`triggerClassName`** / **`triggerStyle`** merge onto the closed trigger **`Button`** (defaults: bordered surface fill); optional **`chevronPosition`** **`"start"`** | **`"end"`** (default **`"end"`**); optional **`chevronVisibility`** **`"always"`** | **`"hover"`** (default **`"always"`**) — uses a named **`group/combobox`** on the trigger so only hover on the combobox shows the chevron (avoids the parent line-card **`group`**); optional **`closedSelectionLabelClassName`** (e.g. **`font-semibold`**) merged onto the closed label **only when a value is selected** — does not affect popover list rows. **`DraftMedicineCard`** uses **`"start"`**, **`"hover"`**, **`closedSelectionLabelClassName`**, and trigger retint on the line-card header bar. First consumer: **`AppointmentPatientCombobox`** (`appointments/_components/`) + **`searchPatientsForPicker`**.

### `<DetailPanel />` — Detail Modal / Page Shell
**Location:** `components/common/DetailPanel.tsx`

Layout for entity create/edit: **header** slot, a **main column** with **tabbed content** (Framer Motion sliding underline on the tab row when more than one tab exists), optional **right sidebar column** (`DetailSidebar` — fixed responsive width `min(26.25rem, 36vw)` capped at **390px**, taking space from the flex-**1** main column; modal shell width unchanged), **footer** with Save, Cancel, and optional destructive action. **Details** tab holds `<DetailForm />` (default on every open; tab state is not URL-persisted). **Documents** / **Appointments** tabs are composed only for roles with `viewDetailSidebar` (admin/doctor); staff see a single Details tab with the tab bar hidden. **Prescriptions** is a separate optional tab gated by **`viewPrescriptions`** (admin/doctor — same role set as documents; not tied to `viewDetailSidebar`): **`PrescriptionsTab`** on appointment detail (edit mode only) and **`PatientPrescriptionsTab`** on patient detail (published rows only). Set **`isCreate`** to hide the right column and give the main column full width.
...
**`DetailPanel` props (entity panels):** **`header`**, **`formRef`**, **`form`** (Details tab), optional **`detailsTabIcon`**, optional **`documentsTab`** / **`appointmentsTab`** / **`prescriptionsTab`** (main-column tab bodies), optional **`sidebarTop`** (passed to **`DetailSidebar`** as **`topSlot`** — appointment **`AppointmentPatientSummaryCard`**), **`events`** / **`hasMoreEvents`** / **`entityType`** / **`entityId`**, **`isCreate`**, footer actions (**`onSave`**, **`onCancel`**, **`onDelete`**, labels).
...
### `<DetailPanelTabs />` — Content-column tab strip
**Location:** `components/common/DetailPanelTabs.tsx`

Client-only helper used inside **`DetailPanel`**. Renders the tab buttons + active panel body. When **`tabs.length === 1`**, the tab strip is omitted entirely. **`resetKey`** changes (entity id / create vs edit) reset the active tab to **Details**. Active indicator: a single **`motion.div`** with a per-panel unique **`layoutId`** (nested in **`LayoutGroup`**) so the underline slides between tabs.

### `<DetailSidebar />` — Activity Log Column
**Location:** `components/common/DetailSidebar.tsx`

Optional **`topSlot`** (appointment **patient summary** card) and a persistent **Activity Log** zone. When **`topSlot`** is set, sidebar height splits **40% / 60%** (`grid-rows` **2fr / 3fr**): the slot wrapper is **`flex`** + **`h-full`** / **`min-h-0`** so **`topSlot`** can fill the split and delegate vertical scrolling to its own surface (the patient card scrolls inside its rounded border). The activity log occupies the lower **60%**. Without **`topSlot`**, the activity log uses the full column height. Data arrives as SSR page-1 (`entries` / `initialHasMore` props); subsequent pages are fetched client-side via `getEntityActivity`. Manages `allEntries` (append-only), `hasMore`, `pageRef`, and `isFetchingRef` locally; `handleLoadMore` is a stable `useCallback([entityType, entityId])`.
...
**Shared tab bodies** (`components/common/`, rendered inside **`DetailPanel`**’s main column): **`DocumentsTab`** — eyebrow **Documents**, ink-styled **Upload** control, 2-column **`DocumentCard`** grid (each **`DocumentCard`**: `var(--color-glass-fill-data)` + backdrop blur + `var(--shadow-card)` so tiles lift off the tab canvas / modal `surface`; presigned open on the main hit target; admin/doctor inline delete control when **`uploadDocument`** — see `docs/09-File-Upload-Flow.md` §5), **`UploadDocumentDialog`**; props include **`documents`** (`PatientDocument[]`), **`patientId`**, optional **`appointmentId`** (metadata for new uploads linking to the visit — the list remains **all** patient-assigned documents), and **`emptyMessage`**. **`AppointmentListTab`** — **`PatientAppointment`** cards use the same **`var(--color-glass-fill-data)`** + blur + card shadow treatment; accordion **status** and expanded **category / visit type / fee** are read-only **`span`**s sharing the same outer layout as **Open** (**`px-2.5`** shell as **`PatientOpenPill`** — patient snapshot age/gender still use narrower **`badgeClass`** **`Badge`**s only on **`AppointmentPatientSummaryCard`**); **Open** **`button`** unchanged from **`PatientOpenPill`**. Non-current **accordion**: full-width header **`button`** (heading + status; chevron **flush right** on the date/time row); expanded: **`justify-between`** row of tinted chips + **Open** **`button`**, then **Description** and **Clinical notes** (**`text-sm`**); no header/body divider; no accordion-header hover fill. **`currentAppointmentId`** → **flat** row (inline Shadcn **`Badge`** **Current** + blue border + status — no **Open** in the body). **`PrescriptionsTab`** (`components/common/PrescriptionsTab.tsx`) — appointment visit prescription (edit mode only): orchestrates state, server actions, and clear dialog; composes modules from **`components/common/prescriptions/`** — **`draft-prescription-shell.tsx`** (**`DraftPrescriptionShell`** — outer card uses `1px solid var(--color-border)` like line items; expandable **body** slot uses `var(--color-surface-alt)` so the revealed region matches the header strip / notes footer and reads clearly on the frosted shell; **`collapsible`** mode adds a chevron + title **button** that expands/collapses the body + notes with the same **`grid-template-rows` / `0fr`↔`1fr`** animation as **`PublishedPrescriptionAccordion`**, while **Clear** / **Publish Rx** stay separate controls; **`PrescriptionsTab`** passes **`defaultOpen`** when there are line items or the add picker is open), **`draft-prescription-item-card.tsx`** (**`DraftMedicineCard`** / **`DraftSortableCard`**), **`dose-slot-row.tsx`**, **`published-prescription-accordion.tsx`**, **`prescription-published-history-list.tsx`**, **`prescription-tab-helpers.ts`**, **`prescription-slots.ts`**. **Draft** UI: shell header (**Prescription** / Rx# / “Draft · auto-saved” + **Clear** + **Publish Rx**); body stacks **nested** `embedded` line cards, dashed **Add another medicine**, optional **Add Medicine** picker; **border-t** **Notes** footer. Each **`DraftMedicineCard`**: header = line index + **`AsyncSearchCombobox`** (**`searchMedicinesForPicker`**, optional **`triggerClassName` / `triggerStyle`** to flush-tint the trigger) + drag/delete; row below = **Days** + **Remarks** (single-line **`input`**s, same height) with no divider before **`DoseSlotRow`** (debounced qty); **`@dnd-kit`** reorder (**`reorderPrescriptionItems`**). **Published (this visit):** **`PublishedPrescriptionAccordion`** with **`highlightCurrent`** (blue border + inline **`Badge`** **Current** — same blue tokens as **`AppointmentListTab`**) → **`RxClinicalDocument`** (`layout="accordionPanel"`: transparent body so the accordion’s **`var(--color-glass-fill-data)`** shell shows through — avoids a flat **`surface`** panel that matched **`ModalShell`** when expanded; no second **Published …** line in the body — the accordion trigger already shows **`publishedAt`**); no first-add empty flow while published (one Rx per visit). **Other prescriptions:** top border + eyebrow + **`PrescriptionPublishedHistoryList`** (published rows for the patient excluding the open visit’s `appointmentId`; each card `1px solid var(--color-border)` like draft line shells); **`router.refresh()`** after **Publish** so SSR history matches. Props: **`appointmentId`**, **`currentAppointmentId`**, **`initialPrescription`**, **`initialPrescriptionHistory`**. First add uses a standalone picker card; further adds use the shell interior.

  **Optimistic interaction model (`DoseSlotRow` in `prescriptions/dose-slot-row.tsx`):** every interactive within a dose-slot card responds immediately without waiting for the server round-trip. Three draft local-state variables (`draftEnabled`, `draftQty`, `draftMeal`) shadow the server-confirmed props, each paired with a `pending*Ref` that tracks the last-queued value. A single `useEffect([enabled, quantity, meal, itemUpdatedAt])` resets each draft only when the server-confirmed value has caught up to the pending ref target — preventing a mid-flight response from an earlier click from stomping a newer optimistic value. (a) **Enable/disable checkbox:** `draftEnabled` + `pendingEnabledRef`; all visual derivations (border, opacity, icon/label color, stepper `disabled`) use `displayEnabled = draftEnabled ?? enabled`. (b) **Qty stepper:** `draftQty` + `pendingQtyRef`; guards the reset so rapid clicks don't flicker. (c) **BF/AF meal toggle:** `draftMeal` + `pendingMealRef`; same guard prevents mid-flight responses from reverting the pill position. The animated meal pill is a **single always-mounted `<motion.span>`** that slides between left (`x: "0%"`) and right (`x: "100%"`) halves — never conditionally mounted/unmounted — so Framer Motion never queues stale animations on rapid clicks.

  **Focus-guarded text fields (`DraftMedicineCard` in `prescriptions/draft-prescription-item-card.tsx`):** `durationFocusedRef` and `remarksFocusedRef` are set on `onFocus`/`onBlur` of the Days and Remarks **inputs** (same row, above dose slots) respectively. Their `useEffect` reset is gated on `!focusedRef.current`, preventing a debounced save from any other field (e.g. a BF/AF toggle) from wiping text the user is actively typing.

  **Optimistic drag reorder:** `onDragEnd` calls `setRx(...)` with the new item order *before* the `await reorderPrescriptionItems(...)`, so cards hold their positions during the server round-trip. On server error, a second `setRx(...)` rolls back to `prevItems`.

  **Concurrent flush guard (`flushDebounced`):** An `inFlight: Set<string>` ref prevents two overlapping `updatePrescriptionItem` calls for the same `itemId`. If a call is already running, `flushDebounced` exits early; the next debounce cycle will pick up any patches that arrived in the meantime. **`PatientPrescriptionsTab`** — patient detail only: read-only list via shared **`PrescriptionPublishedHistoryList`** (**published** rows — same slice as **`getPatientDetail.prescriptions`** / **`PatientPrescriptionsTabLoader`** / **`getPrescriptionsByPatient`** — drafts excluded); **`patientId`**, **`initialPrescriptions`**; **`RxClinicalDocument`** (`layout="plain"`) — **no navigation** to appointments. Empty state explains drafts are not listed. **`PatientDetailPanel`** and **`AppointmentDetailPanel`** pass **`documentsTab`** / **`appointmentsTab`**; **`AppointmentDetailPanel`** and **`PatientDetailPanel`** pass **`prescriptionsTab`** when **`usePermission("viewPrescriptions")`**.
