...
**Shared tab bodies** (`components/common/`, rendered inside **`DetailPanel`**’s main column): **`DocumentsTab`** — eyebrow **Documents**, ink-styled **Upload** control, 2-column **`DocumentCard`** grid (each **`DocumentCard`**: presigned open on the main hit target; admin/doctor inline delete control when **`uploadDocument`** — see `docs/09-File-Upload-Flow.md` §5), **`UploadDocumentDialog`**; props include **`documents`** (`PatientDocument[]`), **`patientId`**, optional **`appointmentId`** (metadata for new uploads linking to the visit — the list remains **all** patient-assigned documents), and **`emptyMessage`**. **`AppointmentListTab`** — **`PatientAppointment`** cards on `var(--color-surface)` with per-status chips; each row navigates to **`/appointments/view/[id]`**; optional **`currentAppointmentId`** shows a **Current** badge and blue border on that row (used only on appointment detail). **`PrescriptionsTab`** (`components/common/PrescriptionsTab.tsx`) — appointment visit prescription (edit mode only): orchestrates state, server actions, and clear dialog; composes modules from **`components/common/prescriptions/`** — **`draft-prescription-shell.tsx`** (**`DraftPrescriptionShell`** — outer card uses `1px solid var(--color-border)` like line items; **`collapsible`** mode adds a chevron + title **button** that expands/collapses the body + notes with the same **`grid-template-rows` / `0fr`↔`1fr`** animation as **`PublishedPrescriptionAccordion`**, while **Clear** / **Publish Rx** stay separate controls; **`PrescriptionsTab`** passes **`defaultOpen`** when there are line items or the add picker is open), **`draft-prescription-item-card.tsx`** (**`DraftMedicineCard`** / **`DraftSortableCard`**), **`dose-slot-row.tsx`**, **`published-prescription-accordion.tsx`**, **`prescription-published-history-list.tsx`**, **`prescription-tab-helpers.ts`**, **`prescription-slots.ts`**. **Draft** UI: shell header (**Prescription** / Rx# / “Draft · auto-saved” + **Clear** + **Publish Rx**); body stacks **nested** `embedded` line cards, dashed **Add another medicine**, optional **Add Medicine** picker; **border-t** **Notes** footer. Line items: **`AsyncSearchCombobox`** + **`searchMedicinesForPicker`**, dose slots (debounced qty), duration/remarks, **`@dnd-kit`** reorder (**`reorderPrescriptionItems`**). **Published (this visit):** **`PublishedPrescriptionAccordion`** with **`highlightCurrent`** (blue border + **Current** badge — same tokens as **`AppointmentListTab`**) → **`RxClinicalDocument`**; no first-add empty flow while published (one Rx per visit). **Other prescriptions:** top border + eyebrow + **`PrescriptionPublishedHistoryList`** (published rows for the patient excluding the open visit’s `appointmentId`; each card `1px solid var(--color-border)` like draft line shells); **`router.refresh()`** after **Publish** so SSR history matches. Props: **`appointmentId`**, **`currentAppointmentId`**, **`initialPrescription`**, **`initialPrescriptionHistory`**. First add uses a standalone picker card; further adds use the shell interior.
  ...
  **Focus-guarded text fields (`DraftMedicineCard` in `prescriptions/draft-prescription-item-card.tsx`):** `durationFocusedRef` and `remarksFocusedRef` are set on `onFocus`/`onBlur` of the Days input and Remarks textarea respectively. Their `useEffect` reset is gated on `!focusedRef.current`, preventing a debounced save from any other field (e.g. a BF/AF toggle) from wiping text the user is actively typing.
