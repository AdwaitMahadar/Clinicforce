...
**Shared tab bodies** (`components/common/`, rendered inside **`DetailPanel`**’s main column): **`DocumentsTab`** — eyebrow **Documents**, ink-styled **Upload** control, 2-column **`DocumentCard`** grid (each **`DocumentCard`**: presigned open on the main hit target; admin/doctor inline delete control when **`uploadDocument`** — see `docs/09-File-Upload-Flow.md` §5), **`UploadDocumentDialog`**; props include **`documents`** (`PatientDocument[]`), **`patientId`**, optional **`appointmentId`** (metadata for new uploads linking to the visit — the list remains **all** patient-assigned documents), and **`emptyMessage`**. **`AppointmentListTab`** — **`PatientAppointment`** cards on `var(--color-surface)` with per-status chips; each row navigates to **`/appointments/view/[id]`**; optional **`currentAppointmentId`** shows a **Current** badge and blue border on that row (used only on appointment detail). **`PrescriptionsTab`** — appointment visit prescription (edit mode only): draft line items with **`AsyncSearchCombobox`** + **`searchMedicinesForPicker`** (excludes medicines already on the Rx), morning/afternoon/night dose-slot rows (qty 1–10 with debounced auto-save, before/after food), duration/remarks, drag-reorder via **`@dnd-kit`** (**`reorderPrescriptionItems`**), prescription-level notes, **Clear medicines** (confirm) + **Publish**; published state is read-only with a **Published** badge. Errors surface via Sonner only — slot toggles save silently. Props: **`appointmentId`**, **`initialPrescription`** (`PrescriptionForAppointmentTab | null` from **`getAppointmentDetail`**).
  ...
  **Concurrent flush guard (`flushDebounced`):** An `inFlight: Set<string>` ref prevents two overlapping `updatePrescriptionItem` calls for the same `itemId`. If a call is already running, `flushDebounced` exits early; the next debounce cycle will pick up any patches that arrived in the meantime. **`PatientPrescriptionsTab`** — patient detail only: read-only rows (**published** prescriptions from **`getPatientDetail.prescriptions`** / **`getPrescriptionsByPatient`** — drafts excluded); **`patientId`**, **`initialPrescriptions`**; each row → **`/appointments/view/[id]`**; empty state explains drafts are not listed. **`PatientDetailPanel`** and **`AppointmentDetailPanel`** pass **`documentsTab`** / **`appointmentsTab`**; **`AppointmentDetailPanel`** and **`PatientDetailPanel`** pass **`prescriptionsTab`** when **`usePermission("viewPrescriptions")`**.
