...
Layout for entity create/edit: **header** slot, a **main column** with **tabbed content** (Framer Motion sliding underline on the tab row when more than one tab exists), optional **right sidebar column** (`DetailSidebar` — fixed responsive width `min(26.25rem, 36vw)` capped at **390px**, taking space from the flex-**1** main column; modal shell width unchanged), **footer** with Save, Cancel, and optional destructive action. **Details** tab holds `<DetailForm />` (default on every open; tab state is not URL-persisted). **Documents** / **Appointments** tabs are composed only for roles with `viewDetailSidebar` (admin/doctor); staff see a single Details tab with the tab bar hidden. Set **`isCreate`** to hide the right column and give the main column full width.

**`DetailPanel` props (entity panels):** **`header`**, **`formRef`**, **`form`** (Details tab), optional **`detailsTabIcon`**, optional **`documentsTab`** / **`appointmentsTab`** (main-column tab bodies), optional **`sidebarTop`** (passed to **`DetailSidebar`** as **`topSlot`** — appointment **`AppointmentPatientSummaryCard`**), **`events`** / **`hasMoreEvents`** / **`entityType`** / **`entityId`**, **`isCreate`**, footer actions (**`onSave`**, **`onCancel`**, **`onDelete`**, labels).
