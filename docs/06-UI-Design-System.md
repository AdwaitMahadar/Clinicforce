...
Optional **`topSlot`** (appointment **patient summary** card) and a persistent **Activity Log** zone. When **`topSlot`** is set, sidebar height splits **40% / 60%** (`grid-rows` **2fr / 3fr**): the slot area scrolls internally on overflow; the activity log occupies the lower **60%**. Without **`topSlot`**, the activity log uses the full column height. Data arrives as SSR page-1 (`entries` / `initialHasMore` props); subsequent pages are fetched client-side via `getEntityActivity`. Manages `allEntries` (append-only), `hasMore`, `pageRef`, and `isFetchingRef` locally; `handleLoadMore` is a stable `useCallback([entityType, entityId])`.
