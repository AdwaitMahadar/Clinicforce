...
- **Right column (edit only, admin/doctor):** **`AppointmentPatientSummaryCard`** above the activity log — from **`getAppointmentDetailCore`** / **`buildAppointmentDetailCore`** (same snapshot shape as the former monolithic mapper): header **`items-stretch`** with **`InitialsBadge`** (`size="md"`, **`size-full`**) inside **`aspect-square`** / **`h-full`** so the square matches the text column height; patient name **`text-[15px]`** bold; compact **age** / **gender** pills (`text-[10px]`, tighter padding) under the name with **`mt-1.5`** so overall header height stays stable; after **gender**, **`PatientOpenPill`** (**`ExternalLink`** + “Open”; same **`button`** classes as **`AppointmentListTab`** expanded-row **Open** → **`/patients/view/[patientId]`**). **Gender** pill keeps **`surface-alt`** fill but tints **label + icon + outline** from tokens: **Male** — **Mars** + **`--color-blue`** / **`--color-blue-border-emphasis`**; **Female** — **Venus** + **`--color-purple`** / **`--color-purple-border`**; **Other** / **Prefer not to say** — **UserRound** + **`--color-amber`** / **`--color-amber-border`**; unknown gender stays neutral border/text. Horizontal rule; **Allergies** and **Past history** sections (optional values **—** when null/empty; past history redacted for staff at the action layer though staff never see this column). **`bloodGroup`** remains in **`patientSummary`** for API/mapper parity but is **not** shown on this card. The card **`h-full`** fills the upper sidebar split and **scrolls inside** its bordered shell when content overflows. **`DetailSidebar`**: **40% / 60%** vertical split between card zone and activity log. Activity log via `events`. Create mode (`isCreate`) hides the right column; staff never see it (`viewDetailSidebar`).
...
#### Prescriptions tab — QA checklist (admin/doctor, local)

Run **`pnpm exec tsc --noEmit`** after prescription-area changes. Before release, spot-check:

| Check | Expected |
|-------|----------|
| **Staff** | No Prescriptions tab; no prescription payloads on appointment detail reads. |
| **No Rx for visit** | Empty state + **Add medicine** when the patient has no draft/published row for this appointment; **Other prescriptions** lists published rows for other visits (and none for a new patient). |
| **Draft** | Collapsible draft shell; **Other prescriptions** excludes only the open visit (draft rows never appear in history). |
| **Published (this visit)** | Top accordion shows **Current** + blue outline; no second-Rx add flow; open visit is **not** repeated under **Other prescriptions**. |
| **After Publish** | **`revalidatePath`** on **`/appointments/view/[id]`** (see `docs/04` — Prescriptions); tab calls **`router.refresh()`** so **`prescriptionHistory`** SSR matches. |
...
- **Output:**
  ```ts
  {
    ...
    patientAppointments: PatientAppointmentSummary[]; // active rows for patient, `scheduled_at` DESC; tab actions redact `title` (no `viewAppointmentTitle`), `notes` (no `viewClinicalNotes`), and staff `fee` until `status === 'completed'`, matching `getPatientDetail` / detail-tab slices
    ...
    prescriptionHistory: PatientPrescriptionSummary[];  // admin/doctor: `getPrescriptionsByPatient({ patientId })` (published only); staff: []
    createdAt:          string;
...
- **RBAC:** All roles for the appointment row; **`prescription`** and **`prescriptionHistory`** are populated only when **`hasPermission(..., "viewPrescriptions")`** (admin/doctor); staff receive **`prescription: null`** and **`prescriptionHistory: []`**.
...
- **Details tab:** `<DetailForm />` with all patient fields, including **Date of Birth** and **Age** (UI-only age; changing DOB updates age immediately; changing age updates approximate DOB after **300ms** debounce as Jan 1 of `current year − age`, with a guard so programmatic age-from-DOB and already-matching pairs do not force Jan 1 on load; **`beforeFormValidate`** on **`DetailForm`** still flushes age→DOB on submit when **`dirtyFields.age`** and values are not already consistent). Then **Gender** on its own row (`colSpan: 2` + `constrainControlToHalfRow` so the control matches one column width), then **Phone** and **Email** side-by-side, then **Blood Group** and **Allergies**, and, for admin/doctor, a tall **Patient's Past History** (`pastHistoryNotes`) textarea (`rows` + `min-height` via `DetailForm` textarea `className`) at the bottom. Create mode uses `createPatientSchema`; view/edit uses `updatePatientSchema`. `PatientDobAgeSync` + **`PatientAgeFormControl`** live in **`DetailForm` `insideForm`** / custom age field (`patients/_components/PatientDobAgeSync.tsx`).
- **Documents / Appointments / Prescriptions tabs (admin/doctor):** Shared **`DocumentsTab`** / **`AppointmentListTab`** / **`PatientPrescriptionsTab`** — first two follow `viewDetailSidebar`; **Prescriptions** follows **`viewPrescriptions`** (see **`DetailPanel`**). Tab order: **Details** → **Documents** → **Appointments** → **Prescriptions**. **`PatientPrescriptionsTab`** lists **published** prescriptions only (`getPrescriptionsByPatient` — drafts are excluded) via shared **`PrescriptionPublishedHistoryList`** — each entry is a **collapsible accordion card** (collapsed: doctor name, Rx ref, visit date, medicine count; expanded: inline read-only ℞ with dose slots and notes) — **no navigation to `/appointments/view/[id]`**. **`AppointmentListTab`** — non-current: **accordion** (full-width **`button`** header; top row: heading + **status** read-only **`span`** chip, same **`px-2.5`** shell as **Open**); **chevron** on the **date/time** row, **`justify-between`** so the chevron sits flush right; expanded: one **`justify-between`** row — three more **`span`** chips (category / visit type / fee) + **`PatientOpenPill`-style** **Open** **`button`**, then **Description** and **Clinical notes**; **`px-4 pb-4 pt-3 space-y-3`** — **no** rule line between header and body; accordion header has **no** hover fill). **`currentAppointmentId`** (**appointment** detail only): **flat** card — inline Shadcn **`Badge`** **Current** + blue border + status only (**no** **Open** in the body, no chevron, no expand); same **`Badge`** props/tokens as **`PublishedPrescriptionAccordion`** **`highlightCurrent`**. On **patient** detail, omit **`currentAppointmentId`**. In **create** mode (`isCreate`), the right column is hidden and only the Details body is shown (tab bar hidden).
...
- **Output:**
  ...
  ```
  **`PatientPrescriptionSummaryAction`** matches **`getPrescriptionsByPatient`** / `lib/actions/prescriptions.ts` **`PatientPrescriptionSummary`**: **`id`**, **`chartId`**, **`appointmentId`**, **`scheduledAt`** (Date — linked visit), **`doctorName`**, **`activeItemCount`**, **`publishedAt`**, **`items`** (`PrescriptionItemForAppointmentTab[]` — full medicine line items fetched alongside the summary so the patient-tab accordion can render the inline Rx document without a separate fetch). The patient-detail mapper exposes ISO strings on **`types/patient.ts`** **`PatientPrescriptionSummary`** for **`PatientPrescriptionsTab`**. Draft prescriptions never appear in this array.

  where `PatientAppointmentSummary` (query layer from `getPatientAppointmentSummaries`; UI maps via `mapAppointmentSummaryRowsToPatientAppointments` → `PatientAppointment`: `scheduledAt` → date/time strings, `heading` via `formatAppointmentHeading`, `notes` → `clinicalNotes`):
  ```ts
  {
    id:           string;
    title:        string | null;
  ...
  ```
  Detail-tab actions (`getPatientDetailAppointmentsTab`, `getAppointmentDetailAppointmentsTab`) apply the same **`title` / `notes` / staff `fee`** redaction as other appointment reads before the UI mapper runs.
  and `DocumentSummary`:
  ```ts
  {
    id:           string;
...
- **`getPatientDetail`**, **`getAppointmentDetail`**, **`getMedicineDetail`**: call `getEntityActivity` after the main entity query (appointments/patients **view** routes use **`loadPatientDetailViewData`** / **`loadAppointmentDetailViewData`**, which call **`get*DetailCore`** and therefore embed the same activity rules on the blocking shell). Staff path returns `activityLog: []` + `activityLogHasMore: false` and skips the DB read. First page (20 entries) + `hasMore` are included in the SSR response.
...
| `/appointments/view/[id]` | Appt detail | `AppointmentDetailPanel mode="edit"` | **`loadAppointmentDetailViewData`** (core + doctors), **`AppointmentDetailPrefetchGroup`**, `*TabLoader` + **`lib/detail-tab-fetch-cache.ts`**, `updateAppointment`, `deleteAppointment` |
...
| `/patients/view/[id]` | Patient detail | `PatientDetailPanel mode="view"` | **`loadPatientDetailViewData`**, **`PatientDetailPrefetchGroup`**, `*TabLoader` + **`lib/detail-tab-fetch-cache.ts`**, `updatePatient`, `getViewPresignedUrl` |
