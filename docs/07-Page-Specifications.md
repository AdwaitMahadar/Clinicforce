...
- **Right column (edit only, admin/doctor):** **`AppointmentPatientSummaryCard`** above the activity log — from **`getAppointmentDetail`** / **`buildAppointmentDetail`**: **`InitialsBadge`** (same fixed square as the patients table, `size="md"`) beside name + **age** and **gender** pill badges **horizontal** under the name (gender uses **Mars** / **Venus** / **UserRound** icons); horizontal rule; **Allergies** and **Past history** sections (optional values **—** when null/empty; past history redacted for staff at the action layer though staff never see this column). **`bloodGroup`** remains in **`patientSummary`** for API/mapper parity but is **not** shown on this card. The card **`h-full`** fills the upper sidebar split and **scrolls inside** its bordered shell when content overflows. **`DetailSidebar`**: **40% / 60%** vertical split between card zone and activity log. Activity log via `events`. Create mode (`isCreate`) hides the right column; staff never see it (`viewDetailSidebar`).
