...
- **Sidebar (edit only):** **Documents** tab (all documents assigned to the patient — same query as patient detail; shared **`DocumentsTab`**; upload passes `appointmentId` + `patientId` so new files link to this visit) + **Appointments** tab (shared **`AppointmentListTab`** — all active appointments for that patient, newest first; current visit has a **Current** label and blue border via **`currentAppointmentId`**; every row, including the current visit, navigates to `/appointments/view/[id]`) + activity log in the sidebar bottom zone (`events`). Create mode (`isCreate`) hides the sidebar.
...
- **Sidebar:** `DetailSidebar` tabbed — Documents | Appointments. Shared **`DocumentsTab`** / **`AppointmentListTab`** (`components/common/`): uploads use **`patientId`** only on patient detail; **Appointments** rows navigate to **`/appointments/view/[id]`** (no **Current** badge — omit **`currentAppointmentId`**). In **create** mode (`isCreate`), the sidebar column is hidden
