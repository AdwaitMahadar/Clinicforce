...
- **Clinical notes / patient past history — staff hidden:** `viewClinicalNotes` is `["admin", "doctor"]`. `PatientDetailPanel` filters `pastHistoryNotes`; `AppointmentDetailPanel` filters appointment `notes` when the user lacks this permission. **`getPatientDetail`**, **`getAppointmentDetail`**, **`getAppointments`**, **`createPatient`**, **`updatePatient`**, **`createAppointment`**, and **`updateAppointment`** also redact or ignore these fields for **staff** so values are not returned or mutated by that role.
- **Appointment title (form field) — staff hidden:** `viewAppointmentTitle` is `["admin", "doctor"]`. `AppointmentDetailPanel` omits the **Title** field when the user lacks this permission (create and edit). **`createAppointment`** stores `title` as null for staff regardless of payload; **`updateAppointment`** strips **`title`** from staff payloads so existing titles cannot be changed or cleared by that role.
