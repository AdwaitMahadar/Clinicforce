...
- **Clinical notes / patient past history — staff hidden:** `viewClinicalNotes` is `["admin", "doctor"]`. `PatientDetailPanel` filters `pastHistoryNotes`; `AppointmentDetailPanel` filters appointment `notes` when the user lacks this permission. **`getPatientDetail`**, **`getAppointmentDetail`** (including joined **`patientPastHistoryNotes`** on the appointment aggregate), **`getPatientDetailAppointmentsTab`**, **`getAppointmentDetailAppointmentsTab`** (per-row **`notes`** in the patient’s visit list), **`getAppointments`**, **`createPatient`**, **`updatePatient`**, **`createAppointment`**, and **`updateAppointment`** also redact or ignore these fields for **staff** (or when the permission is absent) so values are not returned or mutated by that role.
