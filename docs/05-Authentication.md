...
- `app/(app)/layout.tsx` builds the public clinic logo URL from `S3_ENDPOINT`, `S3_BUCKET_NAME`, and `session.user.clinicSubdomain` (`lib/clinic/build-clinic-logo-url.ts` — `{subdomain}/assets/logo/logo.png`) and passes it to `SideNav`; the sidebar shows `InitialsBadge` until the logo `<img>` successfully loads (`onLoad`); the image stays hidden until then so missing files, CORS, or network errors never show a broken icon
