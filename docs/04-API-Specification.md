...
Tab slice placeholders: **`getSettingsDetailTemplatesTab`**, **`getSettingsDetailIntegrationsTab`** — all roles; used with **`lib/detail-tab-fetch-cache.ts`**.

| Action | Schema | RBAC / behaviour |
|--------|--------|------------------|
| **`updateUserTheme`** | `userThemeSchema` (`lib/validators/settings.ts`) | **`requireRole`** all roles. Merges `{ theme }` into **`users.preferences`**. **Cache:** `revalidatePath` on app shell paths (see action). |
| **`updateClinicSettingsColors`** | `updateClinicSettingsColorsSchema` | **`hasPermission("editClinicThemeColors")`** (admin + doctor). Merges optional **`primaryColor`** / **`secondaryColor`** (6-digit hex) into **`clinics.settings`** for the session clinic. |
| **`resetClinicColorsToDefault`** | — | **`editClinicThemeColors`**. Sets **`primaryColor`/`secondaryColor`** to **`defaultPrimaryColor`/`defaultSecondaryColor`**. |
| **`setClinicThemeColorDefaults`** | — | **`setClinicThemeDefaults`** (admin). Copies current primary/secondary into the default fields. |
| **`getClinicLogoUploadPresignedUrl`** | `getClinicLogoUploadPresignedUrlSchema` | **`manageClinicLogo`**. PNG only; max **2 MB**; key **`{subdomain}/assets/logo/logo.png`** (`lib/storage/clinic-logo-object-key.ts`); `PutObject` presign (15 min). |
| **`confirmClinicLogoUpload`** | — | **`manageClinicLogo`**. Sets **`clinics.settings.logoUpdatedAt`** to ISO time (cache buster for public logo URL in **`buildClinicLogoPublicUrl`**). |
| **`deleteClinicLogo`** | — | **`manageClinicLogo`**. **`DeleteObject`** S3 first; on success clears **`logoUpdatedAt`** in settings jsonb. |

`getSession()` exposes **`user.preferences`** and **`user.clinic.settings`** for the authenticated layout and **`ClinicAppearanceProvider`** (client) — `docs/05-Authentication.md`, `docs/06-UI-Design-System.md` (clinic tokens).
...
**Detail tab slice actions (streaming / prefetch):** `lib/actions/appointments.ts` adds **`getAppointmentDetailDocumentsTab`**, **`getAppointmentDetailAppointmentsTab`**, and **`getAppointmentDetailPrescriptionsTab`** (input: appointment UUID; RBAC and response shapes aligned with the corresponding **`getAppointmentDetail`** fields — staff get empty document / Rx payloads where today’s monolithic action does). `lib/actions/patients.ts` adds **`getPatientDetailDocumentsTab`**, **`getPatientDetailAppointmentsTab`**, and **`getPatientDetailPrescriptionsTab`** (input: patient UUID; aligned with **`getPatientDetail`** tab payloads). **`lib/detail-tab-fetch-cache.ts`** exports **one `React.cache` wrapper per slice** (each wraps a call to the matching tab action); **`lib/parallel-tab-data-prefetch.tsx`** and real tab loaders must import **only** these cached exports — do not define `cache()` inline in other modules or per-request deduplication breaks silently. **`lib/detail-tab-ui-mappers.ts`** maps tab action rows to **`DocumentsTab`** / list / Rx UI shapes (shared with **`buildAppointmentDetail`** / **`buildPatientDetail`**). RSC entrypoints: **`app/(app)/appointments/_components/detail-tabs/`** (`AppointmentDetailPrefetchGroup` + **`Appointment*TabLoader`**) and **`app/(app)/patients/_components/detail-tabs/`** (`PatientDetailPrefetchGroup` + **`Patient*TabLoader`**) and **`app/(app)/settings/_components/detail-tabs/`** (`SettingsDetailPrefetchGroup` + **`Settings*TabLoader`**). **Settings** (`lib/actions/settings.ts`): **`getSettingsDetailTemplatesTab`** / **`getSettingsDetailIntegrationsTab`** return placeholder payloads for non-General tabs; cached as **`fetchSettingsDetailTemplatesTabCached`** / **`fetchSettingsDetailIntegrationsTabCached`** (no id — singleton per request). DB: **`getAppointmentPatientIdForClinic`** in `lib/db/queries/appointments.ts` and **`isPatientInClinic`** in `lib/db/queries/patients.ts` scope tab reads without loading the full **`getAppointmentById`** / **`getPatientById`** aggregates.

**Settings blocking shell:** **`loadSettingsViewData()`** in **`app/(app)/settings/_lib/settings-view-data.ts`** — **`getSession`** + **`requireRole`** for **`admin`/`doctor`/`staff`**, then one join on **`users`** + **`clinics`** for the current user’s **`preferences`** and clinic **`settings`**. Payload includes **`clinicName`**, **`clinicLogoUrl`** (`buildClinicLogoPublicUrl`), **`userType`**, plus **`settings`** / **`preferences`** for **`SettingsGeneralTab`**. Returns **`null`** on failure / forbidden; used by **`app/(app)/settings/page.tsx`** and **`app/(app)/@modal/(.)settings/page.tsx`** (with sibling **`SettingsDetailPrefetchGroup`**) as documented in **`docs/07-Page-Specifications.md`** (Settings).
