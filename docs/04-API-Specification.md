...
**Cache:** Successful mutating actions call **`revalidatePath`** on **`/appointments/dashboard`**, **`/patients/dashboard`**, **`/appointments/view/[appointmentId]`**, and **`/patients/view/[patientId]`** for the linked entities.
...
**Prescriptions (RBAC):** For **staff**, **`getPatientDetail`** returns **`prescriptions: []`**. Admin and doctor receive **`prescriptions`** from **`getPrescriptionsByPatient`** embedded in **`getPatientDetail`** (published rows only — drafts are excluded server-side).
...
**`getAppointmentDetail` aggregate:** In addition to the appointment row, the action returns **`patientDocuments`** (same shape as `getPatientDetail` documents — `getDocumentsByAssignment` for the appointment’s patient; **staff: `[]` in the action response**) and **`patientAppointments`** (same summary shape as `getPatientDetail` appointments — `getPatientAppointmentSummaries`; nested `title` staff-redacted like `getPatientDetail`). The query layer also joins **patient** demographics (`patientDateOfBirth`, `patientGender`, `patientBloodGroup`, `patientAllergies`, `patientPastHistoryNotes`) for the appointment UI **`patientSummary`**; **`patientPastHistoryNotes`** is **`null`** for staff in the action response (parallel to `getPatientDetail`). DB: `getAppointmentById` in `lib/db/queries/appointments.ts` composes `getDocumentsByAssignment` + `getPatientAppointmentSummaries` from `lib/db/queries/patients.ts` / `documents.ts`.

**`buildAppointmentDetail` → `patientSummary`:** UI mapper (`appointments/_lib/appointment-detail-mapper.ts`) trims string fields to **`null`** when blank; **`gender`** is **`PatientGender | null`** ( **`null`** when DB gender is missing or not one of `male` / `female` / `other` — no default label). **`AppointmentPatientSummaryCard`** renders **—** for **`null`**, empty, or whitespace-only display values.
